...
var valuesModule = require("./values");
...
exports.register = function(db, endpoints) {
  var event = require("./event").getter(db),
      ...
      values = valuesModule.getter(db),
      maxValue = valuesModule.maxGetter(db);
  ...
  endpoints.http.push(
    ...
    endpoint("GET", "/1.0/values", valuesGet),
    endpoint("GET", "/1.0/values/get", valuesGet),
    endpoint("GET", "/1.0/values/max", valuesGetMax)
  );
  ...
  function valuesGet(request, response) {
  
    values(url.parse(request.url, true).query, function(data, statusCode) {
  ...
  function valuesGetMax(request, response) {

    maxValue(url.parse(request.url, true).query, function(data, statusCode) {
